...
class LifecycleTests(TestCase):
 ...
 def test_malformed_search_deferred_not_cancelled(self):
  blocked=set();c=self.cand(retry_not_before=self.now+timedelta(hours=1)); counts,events,failed=update_movie_search_candidates(target_instance=self.target,target_movies=[movie(1,'broken')],eligible_movie_ids=[1],confirmed_monitored_ids=[1],submission_blocked_movie_ids=blocked,now=self.now+timedelta(minutes=1));c.refresh_from_db();self.assertTrue(failed);self.assertEqual(blocked,{1});self.assertEqual(c.status,'pending');self.assertIsNotNone(c.retry_not_before);self.assertEqual(counts['search_candidates_deferred'],1);self.assertIn('deferred',events[0])
 ...
 def test_retry_exhausted_resolution_uncertainty_and_fresh_cycle(self):
  c=self.cand(status='failed',attempt_count=3,retry_not_before=self.now+timedelta(hours=1),last_error='retry limit exhausted')
  update_movie_search_candidates(target_instance=self.target,target_movies=[movie(1)],eligible_movie_ids=[1],confirmed_monitored_ids=[],now=self.now);c.refresh_from_db();self.assertEqual((c.status,c.last_error),('failed','retry limit exhausted'))
  counts,events,_=update_movie_search_candidates(target_instance=self.target,target_movies=[movie(1,has=True)],eligible_movie_ids=[],confirmed_monitored_ids=[],now=self.now+timedelta(minutes=1));c.refresh_from_db();self.assertEqual(counts['search_candidates_cancelled'],1);self.assertIn('retry-exhausted candidate resolved',events[0]);self.assertEqual((c.status,c.last_error,c.retry_not_before,c.attempt_count),('cancelled','',None,3))
  update_movie_search_candidates(target_instance=self.target,target_movies=[movie(1)],eligible_movie_ids=[1],confirmed_monitored_ids=[1],now=self.now+timedelta(hours=1));c.refresh_from_db();self.assertEqual((c.status,c.attempt_count,c.current_command_id,c.last_error),('pending',0,None,''))
 def test_retry_exhausted_malformed_and_tmdb_lineage_conflict_fail_closed(self):
  c=self.cand(status='failed',attempt_count=3,last_error='retry limit exhausted');cmd=Command.objects.create(target_instance=self.target,radarr_command_id=9,status='failed',submission_attempted_at=self.now);Link.objects.create(command=cmd,candidate=c,target_movie_id=1);c.current_command=cmd;c.save()
  update_movie_search_candidates(target_instance=self.target,target_movies=[movie(1,'bad')],eligible_movie_ids=[1],confirmed_monitored_ids=[1],now=self.now);c.refresh_from_db();self.assertEqual(c.status,'failed')
  changed=movie(1);changed['tmdbId']=9999;update_movie_search_candidates(target_instance=self.target,target_movies=[changed],eligible_movie_ids=[],confirmed_monitored_ids=[],now=self.now);c.refresh_from_db();self.assertEqual((c.status,c.current_command_id),('failed',cmd.id))
 def test_retry_exhausted_removed_movie_resolves(self):
  c=self.cand(status='failed',attempt_count=3,last_error='retry limit exhausted');counts,events,_=update_movie_search_candidates(target_instance=self.target,target_movies=[],eligible_movie_ids=[],confirmed_monitored_ids=[],now=self.now);c.refresh_from_db();self.assertEqual((c.status,counts['search_candidates_cancelled']),('cancelled',1));self.assertIn('no_longer_eligible',events[0])
 def test_malformed_due_retry_is_blocked_without_lineage_changes(self):
  c=self.cand(); predecessor=self.command(1,8,'failed'); predecessor.outcome_reconciled_at=self.now;predecessor.save();c.refresh_from_db();c.status='pending';c.attempt_count=1;c.retry_not_before=self.now;c.save();blocked=set()
  update_movie_search_candidates(target_instance=self.target,target_movies=[movie(1,'broken')],eligible_movie_ids=[1],confirmed_monitored_ids=[1],submission_blocked_movie_ids=blocked,now=self.now)
  api=Mock();submit_pending_search_candidates(target_api=api,target_instance=self.target,submission_blocked_movie_ids=blocked,now=self.now);c.refresh_from_db();self.assertEqual(c.current_command_id,predecessor.id);self.assertEqual(c.retry_not_before,self.now);self.assertEqual(c.status,'pending');api.trigger_movies_search.assert_not_called()
 ...
 def test_initial_terminal_status_mapping(self):
  for n,(status,result,expected) in enumerate((('completed','successful','completed'),('completed','unsuccessful','failed'),('completed','indeterminate','ambiguous'),('failed','unsuccessful','failed'),('aborted','unsuccessful','aborted'),('cancelled','unsuccessful','cancelled'),('orphaned','unsuccessful','orphaned')),1):
   Candidate.objects.all().delete();Command.objects.all().delete();self.cand(n);api=Mock();api.trigger_movies_search.return_value={'status_code':201,**resource(10+n,[n],status,result)};_,events,_=submit_pending_search_candidates(target_api=api,target_instance=self.target,now=self.now);self.assertEqual(Command.objects.get().status,expected);self.assertIn(f'MoviesSearch {expected} command_id=',events[0])
 ...
 def test_batching_250(self):
  Candidate.objects.bulk_create([Candidate(target_instance=self.target,target_movie_id=i,tmdb_id=1000+i,first_eligible_at=self.now,last_confirmed_at=self.now) for i in range(1,251)]);api=Mock();ids=iter((1,2,3));api.trigger_movies_search.side_effect=lambda batch:{'status_code':201,**resource(next(ids),batch)};counts,_,_=submit_pending_search_candidates(target_api=api,target_instance=self.target,now=self.now);self.assertEqual(counts['submitted'],250);self.assertEqual([len(c.args[0]) for c in api.trigger_movies_search.call_args_list],[100,100,50]);self.assertEqual([c.candidate_links.count() for c in Command.objects.order_by('id')],[100,100,50]);self.assertEqual(set(Command.objects.values_list('submission_attempted_at',flat=True)),{self.now})
 def test_each_production_batch_gets_fresh_timestamp_and_late_adoption(self):
  start=self.now.replace(second=0,microsecond=0); batch_times=[start,start+timedelta(minutes=6),start+timedelta(minutes=14)]
  Candidate.objects.bulk_create([Candidate(target_instance=self.target,target_movie_id=i,tmdb_id=1000+i,first_eligible_at=start,last_confirmed_at=start) for i in range(1,251)]);api=Mock();calls={'count':0}
  def submit(batch):
   calls['count']+=1
   if calls['count']==3:return {'status_code':201,'name':'MoviesSearch'}
   return {'status_code':201,'id':calls['count'],'name':'MoviesSearch','body':{'name':'MoviesSearch','movieIds':batch},'status':'queued','result':'unknown'}
  api.trigger_movies_search.side_effect=submit
  with patch('mdblistrr.radarr_search._submission_time',side_effect=[start,*batch_times]): submit_pending_search_candidates(target_api=api,target_instance=self.target)
  commands=list(Command.objects.order_by('id'));self.assertEqual([c.submission_attempted_at for c in commands],batch_times);late=commands[2];self.assertEqual(late.status,'ambiguous')
  late_ids=list(range(201,251));raw=resource(99,late_ids);raw['queued']=(batch_times[2]+timedelta(minutes=1)).isoformat();parsed=validate_movie_search_command(raw,late_ids)
  reconcile_movie_search_commands(target_instance=self.target,command_map={99:(raw,parsed)},poll_failed=False,target_movies=[movie(i) for i in range(1,251)],eligible_movie_ids=range(1,251),now=batch_times[2]+timedelta(minutes=2));late.refresh_from_db();self.assertEqual(late.radarr_command_id,99);self.assertEqual(late.status,'queued')
