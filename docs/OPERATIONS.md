...
If names remain missing, use **Refresh missing titles** in the detail modal or standalone detail page. This separate, authenticated POST action calls the configured On-Demand Arr's read-only catalogue lookup by exact TVDb/TMDb ID. It does not depend on reconciliation being enabled or succeeding, and can recover titles for items removed from both libraries. Arr may consult its metadata provider; MDBListarr needs no additional metadata-service credentials. Merely viewing Health or opening details remains local-only.

The action selects missing IDs from the displayed page on the server, deduplicates them, and saves verified names on all unnamed search/cleanup records for that target and external ID. It preserves existing titles and every lifecycle field/timestamp. It performs no monitoring, acquisition searches, additions or deletions. Each refresh starts at most 25 lookups within a 20-second work budget, with one GET per ID and a 3-second connection / 5-second read timeout; an in-progress request can finish after the budget. Connection failures stop the batch. The result reports recovered titles, updated records, unresolved matches, failures and deferred work. Use another refresh for deferred titles; catalogue removal, ambiguity or an unavailable Arr metadata provider can still leave names unresolved. No names are guessed from filenames or unrelated IDs.
